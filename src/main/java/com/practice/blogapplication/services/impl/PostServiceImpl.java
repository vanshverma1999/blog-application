...
//        int pageNumber = 1;
    ...
    public List<PostDto> searchPosts(String keyword) {
        List<Post> sPost = this.postRepo.findByTitleContaining(keyword);
        return sPost.stream().map((post)->this.modelMapper.map(post,PostDto.class)).collect(Collectors.toList());
    }
}
